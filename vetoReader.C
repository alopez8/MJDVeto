...
void vetoReader(){
	...
	TFile *f = new TFile("/global/project/projectdirs/majorana/data/mjd/surfmjd/data/built/P3JDY/OR_run6947.root");
	...
	*/
	...
	long Index[32] = {0};
	int k = 0;
	cout << "before loop (Line 198)" << endl;
	for (int j = 0; j<32; j++)	{
		cout << "j = " << j << " vd[j] = " << vd[j] << endl;
		if (vd[j]){
